...
SELECT "name", "governmentform" AS "form of government", "headofstate" AS "head of state" FROM "country" WHERE "continent" = 'Europe';

-- 4. The name, population, surface area, and average life expectancy of all countries in Asia (51 rows)
SELECT "name", "population", "surfacearea" AS "surface area", "lifeexpectancy" AS "average life expectancy" FROM "country" WHERE "continent" = 'Asia';

-- 5. The name, country code, and population of all cities with a population greater than 8 million people (10 rows)
SELECT "name", "countrycode" AS "country code", "population" FROM "city" WHERE "population" > 8000000;

-- 6. The name, country code, and population of all cities with a population less than one thousand people (11 rows)
SELECT "name", "countrycode" AS "country code", "population" FROM "city" WHERE "population" < 1000;

-- 7. The name, continent, and GNP of all countries with a GNP greater than one trillion dollars (6 rows)
SELECT "name", "continent", "gnp" AS "GNP" FROM "country" WHERE "gnp" > 1000000;
...
SELECT "name", "continent", "population", "gnp" AS "GNP", "lifeexpectancy" AS "average life expectancy" FROM "country" WHERE "lifeexpectancy" > 80;
...
SELECT "name", "continent", "headofstate" AS "head of state" FROM "country" WHERE "governmentform" ILIKE '%monarchy%';

-- 13. The name of all cities in the USA with a population between 1 million and 2 million people (6 rows) 
SELECT "name" FROM "city" WHERE "population" BETWEEN 1000000 AND 2000000 AND "countrycode" = 'USA';
...
SELECT "name", "population", "gnp" AS "GNP" FROM "country" WHERE "gnp" > 1000000 AND "population" < 100000000;
...
SELECT "name", "lifeexpectancy" AS "average life expectancy" FROM "country" WHERE "continent" = 'Oceania';
...
SELECT "name", "lifeexpectancy" AS "average life expectancy" FROM "country" WHERE "continent" = 'Oceania' AND "lifeexpectancy" IS NOT NULL;
...
SELECT "name", "continent", "gnp" AS "GNP", "lifeexpectancy" AS "average life expectancy" FROM "country" WHERE "lifeexpectancy" >= 70 AND "gnp" BETWEEN 1 AND 100;

-- 21. The per capita GNP (i.e. GNP divided by population) in US Dollars of all countries in Europe (46 rows)
SELECT "gnp"*1000000/"population" AS "per capita GNP" FROM "country" WHERE "continent" = 'Europe';

-- 22. The number of years since independence for all countries that have a year of independence (192 rows)
SELECT date_part('year', CURRENT_DATE) - "indepyear" AS "number of years since independence" FROM "country" WHERE "indepyear" IS NOT NULL;
